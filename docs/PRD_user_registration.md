...
> **版本:** v1.2-confirmed
> **澄清依据:** docs/PRD_CLARIFICATION_user_registration.md（3 轮 11 问）+ 第2轮深度评审（4-Agent，2026-06-13）
> **创建日期:** 2026-06-13
> **作者:** Hermes 智能研发工作流
> **状态:** 已冻结（v1.2-confirmed，第1轮多视角评审通过 + 4项假设确认 + 第2轮深度审查14项P0已修订）
...
| AC-12 | P0 | 协议链接可点击 | 点击"《用户协议》"打开协议详情页（WebView 加载远程 URL） |
| AC-13 | P1 | "已有账号？去登录"入口 | 注册页提供跳转登录页的入口 |
...
| AC-22 | P0 | 注册成功确认反馈 | 注册成功后显示 Snackbar "注册成功"（绿色），短暂停留后自动登录跳转主页 |
| AC-23 | P0 | 验证码格式校验 | 验证码输入框仅接受 6 位数字，拒绝非数字字符并即时提示；连续 3 次输入错误验证码后提示"验证码错误次数过多，请重新获取" |
| AC-24 | P0 | 提交中可取消 | 注册提交 loading 遮罩期间可按系统返回键取消请求，取消后表单数据保留，按钮恢复可点击 |
| AC-25 | P0 | 提交失败后表单数据保留 | 注册提交失败（网络错误/服务端错误）后，手机号、密码、确认密码、协议勾选状态全部保留，用户可直接修改后重试 |
| AC-26 | P0 | 网络超时独立处理 | 注册请求超时（15 秒）时 Toast 提示"网络超时，请检查网络后重试"，区别于一般服务端错误 |
| AC-27 | P0 | SMS 自动填充降级方案 | SMS Retriever API 可用时自动填充验证码；不可用时（无 GMS 设备）用户手动输入，验证码输入框支持粘贴 |
| AC-28 | P1 | 密码可见状态配置变更保留 | 横竖屏旋转/折叠屏展开后，两个密码框的可见/隐藏状态保持不变 |
...
| 验证码倒计时 | 使用 ViewModel 作用域协程 + StateFlow<Long> 实现 countdown（避免 LaunchedEffect 在 Composable 组合树变更时重置），基于服务器返回的 sentTimestamp 计算剩余秒数 |
| 协程生命周期 | 所有注册/发送验证码协程绑定 viewModelScope，在 ViewModel.onCleared() 时自动取消，防止页面离开后协程泄漏 |
...
| SavedStateHandle 字段 | 需持久化：phone（手机号）、smsSentTimestamp（发送时间戳）、agreementAccepted（协议勾选）；password 不持久化（安全原因，避免明文进入系统内存） |
...
| SMS Retriever API 在国内设备不可用 | 华为/小米等无 GMS 设备无法自动填充验证码，用户体验降级 | 优先尝试 SMS Retriever API，不可用时回退手动输入模式；验证码输入框支持粘贴（Android 自动提取 SMS 码）；后续版本考虑 SMS User Consent API 作为补充 |
| Token 持久化后导航失败 | 注册成功、Token 已写入，但 navigate() 时 Activity 被系统销毁 | App 启动时检测"Token 已存在但未进入主页"状态，自动完成登录跳转；注册成功后优先持久化 Token 再导航 |
...
| 深色模式 | Material3 darkColorScheme + surface/onSurface tokens；error 色 supportingText 深色模式下需满足 WCAG AA 4.5:1 对比度；半透明遮罩深色模式下需加深至 60% 不透明度以确保可见 |
...
| 提交取消 | 遮罩层期间系统返回键可取消请求（ViewModel 取消协程），取消后表单数据保留 |
...
```gherkin
Scenario: TC-19 注册按钮防重复提交
  Given 用户已填写完整有效的注册信息，且已勾选用户协议
  When  用户快速连续点击"注册"按钮两次
  Then  仅发起一次注册网络请求
  And   按钮在请求完成前变为禁用状态并显示"注册中..."
  And   第二次点击不产生额外请求
```

```gherkin
Scenario: TC-20 验证码输入格式错误
  Given 用户在验证码输入框中
  When  用户尝试输入非数字字符（如字母、特殊符号）
  Then  系统阻止该字符输入或即时提示"请输入6位数字验证码"
  And   验证码输入框内容仅保留有效数字
  And   连续 3 次提交错误验证码后提示"验证码错误次数过多，请重新获取"
```

```gherkin
Scenario: TC-21 注册请求网络超时
  Given 用户已填写完整有效的注册信息并点击"注册"按钮
  When  网络请求在 15 秒内未收到服务端响应
  Then  Toast 提示"网络超时，请检查网络后重试"
  And   注册按钮恢复可用状态
  And   已填写的表单数据（手机号、密码等）保留不丢失
  And   用户可重新点击注册发起请求
```

```gherkin
Scenario: TC-22 密码可见状态在配置变更时保留
  Given 用户已将密码切换为可见状态（明文显示）
  When  用户旋转设备屏幕（配置变更）
  Then  密码可见状态保持为"可见"
  And   密码输入内容不丢失
  And   确认密码输入框同理保持其可见性状态
```

```gherkin
Scenario: TC-23 提交失败后表单数据保留
  Given 用户已填写完整的注册信息并点击"注册"按钮
  When  服务端返回注册失败（如 500 错误）
  Then  系统显示明确的失败提示信息
  And   手机号、密码、确认密码内容保留在对应输入框中
  And   协议勾选状态保留
  And   用户可直接修改后重新提交，无需重新填写全部信息
```
...
| TC-19 | 注册按钮防重复提交 | AC-11 | P0 |
| TC-20 | 验证码输入格式错误 | AC-23 | P0 |
| TC-21 | 注册请求网络超时 | AC-26 | P0 |
| TC-22 | 密码可见状态配置变更保留 | AC-17, AC-28 | P1 |
| TC-23 | 提交失败后表单数据保留 | AC-25 | P0 |
...
| 决议编号 | 决议内容 | 来源 | 状态 |
|----------|----------|------|------|
| R-01 | 验证码发送方式：后端统一短信发送 | 假设 → 已确认 | 已确认 |
| R-02 | 协议链接：in-app WebView 加载远程 URL，含本地 fallback | 假设 → 已确认 | 已确认 |
| R-03 | "去登录"自动填入手机号：是，从已注册拦截跳转时自动填入 | 假设 → 已确认 | 已确认 |
| R-04 | 注册页"已有账号？去登录"入口：底部 TextButton | 假设 → 已确认 | 已确认 |
...
> **状态:** 全部决议已确认（R-01~R-04 已确认，D-01~D-14 全部已确认）。PRD v1.1-confirmed 已冻结。

---

### 12.7 第2轮评审（2026-06-13，深度二次审查）

> **评审方式:** 4-Agent 并行（产品/技术/UX/QA），深度审查第1轮遗漏
> **结果:** 发现 14 个 P0、21 个 P1，新增 7 条 AC (AC-22~AC-28)、5 个 Gherkin 场景 (TC-19~TC-23)，已自动修订至 PRD 正文

#### 12.7.1 评审总览

| 视角 | 评分 | P0 项 | P1 项 | 核心发现 |
|------|:----:|:-----:|:-----:|----------|
| 产品视角 | 7.5/10 | 3 | 5 | 注册成功心理闭环缺失、协议优先级错标、验证码容错无边界 |
| 技术视角 | 6.0/10 | 5 | 8 | 架构细节留空严重：竞态条件、协程生命周期、SMS降级、错误封装 |
| UX 视角 | 5.5/10 | 5 | 10 | 零动画定义、Loading无取消、深色对比度未验证、极端尺寸无策略 |
| QA 视角 | — | — | — | 新增 5 个 Gherkin 场景，发现 4 处现有场景可执行性问题 |

#### 12.7.2 产品视角（Agent A，评分 7.5/10）

**P0（已修订到正文）:**
- P0-01: 用户协议优先级 P2→P0（法律合规前置条件）→ AC-12 已升级为 P0
- P0-02: 注册成功无确认反馈 → 新增 AC-22（Snackbar "注册成功"）
- P0-03: 验证码格式/重试上限缺失 → 新增 AC-23（仅数字 + 3次上限）

**P1:**
- P1-01: "去登录"优先级 P2→P1 → AC-13 已升级为 P1
- P1-02: 表单断点续填无 AC → 已由新增 AC-25 覆盖（提交失败后保留）
- P1-03: 中途改手机号后验证码与倒计时状态未定义
- P1-04: SMS 自动填充无场景覆盖 → 已由新增 AC-27 覆盖
- P1-05: 密码弱强度无分层感知

#### 12.7.3 技术视角（Agent B，评分 6.0/10）

**P0（已修订到正文）:**
- P0-01: Token 持久化后导航失败无恢复 → §7 新增风险 + 缓解（App 启动检测）
- P0-02: SavedStateHandle 存 password 明文泄露 → §6 移除 password 字段
- P0-03: ViewModel 协程生命周期未定义 → §6 新增协程生命周期约束
- P0-04: SMS Retriever 国内不可用无降级 → §7 新增风险 + 新增 AC-27
- P0-05: 网络超时未独立处理 → 新增 AC-26（15s 超时）

**P1:**
- P1-01: Repository 层错误封装策略未定义 → 建议统一 Result<T>
- P1-02: UseCase 未命名（零个 UseCase 被列举）
- P1-03: 导航跨页面参数传递未定义
- P1-04: send-sms 幂等性未定义 + 客户端防重
- P1-05: Token 刷新机制未定义
- P1-06: 证书固定未提及
- P1-07: ProGuard 规则未定义（kotlinx.serialization + 敏感字段）
- P1-08: 手机号格式化扩展点（D-08 国际版预留）

#### 12.7.4 UX 视角（Agent C，评分 5.5/10）

**P0（已修订到正文）:**
- P0-01: Loading 遮罩无取消策略 → §9.6 新增提交取消约束 + 新增 AC-24
- P0-02: SMS 自动填充无降级 → 新增 AC-27（手动输入 + 粘贴）
- P0-03: 提交失败后表单保留不明确 → 新增 AC-25 + 新增 TC-23
- P0-04: 深色模式 error 色对比度未验证 → §9.6 深色约束追加 WCAG AA 4.5:1
- P0-05: 密码可见状态配置变更不保留 → 新增 AC-28 + 新增 TC-22

**P1:**
- P1-01: 零动画定义（loading/Snackbar/过渡/键盘均无 motion spec）
- P1-02: 折叠屏/平板布局策略空白（仅覆盖 480dp 横屏）
- P1-03: TalkBack 无障碍不完整（Checkbox/倒计时按钮/loading 缺 contentDescription）
- P1-04: 验证码单行输入 vs 6 方框体验差异
- P1-05: 密码强度实时指示器缺失
- P1-06: App 后台恢复时序未定义
- P1-07: 429 速率限制状态缺失
- P1-08: 网络超时与服务器错误未区分
- P1-09: 协议链接交互方式（内置 WebView vs 外部浏览器）未明确
- P1-10: Material You 动态取色未提及

#### 12.7.5 QA 视角（Agent D，增量覆盖）

**新增 Gherkin 场景（5 个）:**

| 场景 | 覆盖 AC | 优先级 | 覆盖的三方 P0 |
|------|:------:|:------:|--------------|
| TC-19 注册按钮防重复提交 | AC-11 | P0 | 填补 AC-11 直接覆盖缺口 |
| TC-20 验证码输入格式错误 | AC-23 | P0 | 产品 P0#3 |
| TC-21 注册请求网络超时 | AC-26 | P0 | 技术 P0#5 |
| TC-22 密码可见状态配置变更保留 | AC-17, AC-28 | P1 | UX P0#5 |
| TC-23 提交失败后表单数据保留 | AC-25 | P0 | UX P0#3 |

**现有场景可执行性问题:**
- TC-04 网络异常未区分"发验证码时无网络"和"提交注册时无网络"
- TC-17 横竖屏状态保留范围不明确
- TC-08/09 手机号格式仅覆盖 2 个边界，缺少空输入/含空格/超 11 位
- AC-21 无障碍零 Gherkin 覆盖

#### 12.7.6 第2轮讨论决议

| 决议编号 | 决议内容 | 来源 | 状态 |
|----------|----------|------|------|
| R2-D-15 | 协议优先级修正：AC-12 P2→P0（法律合规前置），AC-13 P2→P1（核心逃逸路径） | 产品 P0 | 已修订 |
| R2-D-16 | 注册成功反馈：Snackbar "注册成功"（绿色），短暂停留后自动登录跳转 | 产品 P0 + AC-22 | 已修订 |
| R2-D-17 | 验证码格式/防暴力：仅接受6位数字，连续3次错误提示重新获取 | 产品 P0 + AC-23 | 已修订 |
| R2-D-18 | Loading 可取消：遮罩期间系统返回键取消请求，表单数据保留 | UX P0 + AC-24 | 已修订 |
| R2-D-19 | 提交失败表单保留：所有错误路径（网络/服务端）保留已填数据 | UX P0 + AC-25 + TC-23 | 已修订 |
| R2-D-20 | 网络超时独立处理：15s 超时提示，区别于服务端错误 | 技术 P0 + AC-26 | 已修订 |
| R2-D-21 | SMS 自动填充降级：SMS Retriever 优先，不可用时手动输入+粘贴 | 技术/UX 共识 P0 + AC-27 | 已修订 |
| R2-D-22 | 密码可见状态保留：配置变更后两个密码框可见/隐藏状态不变 | UX P0 + AC-28 + TC-22 | 已修订 |
| R2-D-23 | SavedStateHandle 安全：移除 password 字段，仅持久化 phone/smsSentTimestamp/agreementAccepted | 技术 P0 + §6 | 已修订 |
| R2-D-24 | 协程生命周期：所有协程绑定 viewModelScope，onCleared() 自动取消；倒计时使用 ViewModel 协程替代 LaunchedEffect | 技术 P0 + §6 | 已修订 |
| R2-D-25 | Token 持久化后导航恢复：App 启动时检测 Token 已存在但未进主页，自动登录跳转 | 技术 P0 + §7 | 已修订 |
| R2-D-26 | 深色模式对比度：error supportingText 满足 WCAG AA 4.5:1，遮罩深色模式 60% 不透明度 | UX P0 + §9.6 | 已修订 |

---

> **第2轮评审结论:** 第1轮在功能表层完备（21 AC / 18 Gherkin），但第2轮深度审查发现大量运行时行为定义空白和技术实现细节缺失。14 个 P0 问题已全部自动修订到 PRD 正文（§4/§6/§7/§9/§10），新增 7 条 AC、5 个 Gherkin 场景、12 项决议。PRD AC 总量 21→28，Gherkin 场景 18→23。建议确认为 v1.2-confirmed。
